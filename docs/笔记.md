...
10. 开发 EditHeader 组件：
    ...
    - 返回
    - 删除：删除选中的组件；计算下一个选中的组件
    - 隐藏/显示：组件信息新增 isHidden 属性（server、redux）；点击修改选中组件的 isHidden 属性；在计算下一个选中的组件以及画布中过滤掉需要隐藏的组件
    - 锁定/解锁：组件信息新增 isLocked 属性（server、redux）；在画布中设置被锁定的组件的样式；如果组件被锁定，那么禁用该组件的属性配置表单。
    - 复制粘贴：在 redux 中存储复制的内容 copiedComponent；当 copiedComponent 为 null 时禁用粘贴按钮；粘贴按钮，插入复制的组件（抽离插入新组件逻辑）
    - 快捷键操作：删除、复制、粘贴、选中上/下一个组件

11. 扩展组件：
    - Paragraph：流程和 Input/Title 一致；输入框换行不要使用 replaceAll + dangerouslySetInnerHTML（不安全），用 split + map 循环实现换行
    - 类似的新增 QuestionInfo、QuestionTextarea、QuestionRadio、QuestionCheckbox 等组件
12. 开发图层组件
13. 开发页面信息组件
14. 开发修改问卷标题、保存、发布等功能
15. 组件列表拖拽排序
    - 技术选型：React-dnd，React-beautiful-dnd，Sortable.js，React-sortable-hoc，**Dnd-kit**（更新及时、star 多、下载量多、demo 简单易懂）
    - 封装独立的组件
    - 解决 Dnd-kit 与快捷键的冲突
16. 上移/下移组件：调用拖拽排序的 action，传入新旧 index 即可
17. 撤销重做：
    - 原理：
    ```js
    // 三个数组，模拟输入框的输入、撤销、重做的过程
    present = "abc1"; // 当前输入框的值
    past = ["a", "ab", "abc", "abc1"]; // 历史数据的记录
    future = ["abcd"]; // 未来重做的数据
    /**
     * 逻辑分析：
     * present 永远显示 past 的最后一个元素
     * 1. 输入：past 入栈；future 清空
     * 2. 撤销：past 出栈；future 入栈；present 重新赋值
     * 3. 重做：future 出栈；past 入栈；present 重新赋值
     */
    ```
    - 自己实现 redux 的撤销重做过于复杂，这里使用插件 **redux-undo**
    - 1. 先改造 components store
    - 2. 再修改 useGetComponents 获取 present 数据
    - 3. 调用 redux-undo 的 action，实现功能键和快捷键实现撤销重做

## 开发问卷统计页

1. 页面布局、未发布提示
2. Header 工具栏：
   - 拷贝链接：`inputElem.select();` // 选中 input 的内容；`document.execCommand("copy");` // 拷贝选中的内容
   - 生成二维码：使用 `qrcode.react` 插件
3. 左侧数组列表组件
4. 创建获取答卷列表接口（模拟数据）
   ...
   ```json
   {
     ...
     "data": {
       ...
       "list": [
         // 单份答卷
         {
           "_id": "120000201611077946", // _id 答卷的id
           "c3": "四则办行放", // c3是对应组件的fe_id
           "c4": "一习业南别",
           "c5": "看走它但种",
           "c7": "选项1",
           "c8": "选项1,选项2"
...
2.  创建 `form` 表单（配置好`method`、`action`属性），通过隐式输入框提交 `questionId`
3.  创建提交按钮
4.  提交表单：
